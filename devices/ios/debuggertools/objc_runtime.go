// Package debuggertools provides tools for interacting with iOS processes via
// the debug proxy and GDB RSP — ObjC runtime calls, memory access, and view
// hierarchy capture, with no external tools (LLDB, Xcode) required.
//
// Vendored from github.com/danielpaulus/go-ios/ios/debuggertools
// (commit 5d5f0d1129b8), which is not part of the released go-ios v1.0.211 that
// the rest of this module depends on. Original code is MIT licensed — see the
// LICENSE file in this directory. NewObjCRuntime / Cleanup are exported here for
// use by the real-device webview path (devices/ios_device_webview.go).
