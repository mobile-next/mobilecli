...
* Feat(ios): `screencapture` and `screenrecord` share one avc stream, so both can run at the same time ([#429](https://github.com/mobile-next/mobilecli/pull/429))
* Fix(ios): Broadcast cold start on real devices: breadcrumb button, picker in the foreground, slow UI dump ([#431](https://github.com/mobile-next/mobilecli/pull/431))
* Fix(ios): `apps install --force-resign` signs the loose dylibs of Xcode debug builds (`<Name>.debug.dylib`, `__preview.dylib`), which used to crash at launch as unsigned ([#433](https://github.com/mobile-next/mobilecli/pull/433))
* Perf(ios): Flutter detection and VM service discovery on real devices without LLDB ([#432](https://github.com/mobile-next/mobilecli/pull/432))
* Fix(android): Screen capture works on Android 12 and 13, where the hidden `createVirtualDisplay` is missing, by falling back to `SurfaceControl.createDisplay` ([#435](https://github.com/mobile-next/mobilecli/pull/435))
* Fix(android): Pick the local port for `adb forward` ourselves instead of `tcp:0`, so agents are reachable when the adb server runs on another host ([#434](https://github.com/mobile-next/mobilecli/pull/434))
* Fix(android): A viewer that joins an idle screen gets a key frame right away instead of waiting for the screen to change ([#437](https://github.com/mobile-next/mobilecli/pull/437))
...
* Feat: Run device commands through a background daemon over a unix socket for major performance boost and keeping tunnels alive ([#384](https://github.com/mobile-next/mobilecli/pull/384))
...
* General: Allocate remote Android and iOS devices on Mobile Fleet, see https://mobilenext.ai ([#176](https://github.com/mobile-next/mobilecli/pull/176))
